...
def request_json(url: str):
    req = Request(
        ...
        headers={
            ...
            "User-Agent": "technocore-chat/1.4",
        },
    )
    ...
    def process_messages(self, payload, initial=False) -> None:
        ...
        for seq, message in normalized:
            ...
            if seq <= current_last_seq:
                continue
            new_messages.append((seq, timestamp, sender, text))
        ...
        if initial:
            new_messages = new_messages[-INITIAL_READ_LIMIT:]

        for item in new_messages:
            with self.queue_lock:
                if len(self.display_queue) >= MAX_QUEUE_SIZE:
                    self.display_queue.popleft()
                self.display_queue.append(item)

        if initial:
            self.write_message("")
            self.write_message(f"Loaded {len(new_messages)} public messages.")
    ...
    def flush_display_queue(self) -> None:
        ...
        with self.queue_lock:
            if not self.display_queue:
                return
            seq, timestamp, sender, text = self.display_queue.popleft()
