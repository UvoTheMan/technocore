...
def derive_did(key: Ed25519PrivateKey) -> str:
    ...
    return "did:key:z" + base58btc_encode(b"\xed\x01" + raw_public)
...
def read_room(room: str, *, since=None, limit=READ_LIMIT, wait=None):
    ...
    if wait is not None and since is not None:
        query["wait"] = str(wait)
    url = f"https://technocore.chat/r/{quote(room, safe='')}?" + urlencode(query)
    req = Request(url, headers={"Accept": "application/json", "User-Agent": "technocore-chat/1.2"})
...
def send_signed_message(identity: Ed25519PrivateKey, did: str, room: str, text: str):
    ...
    url = (
        f"https://technocore.chat/r/{quote(room, safe='')}/say-signed/"
        f"{quote(did, safe='')}/{quote(signature, safe='')}/{nonce}/{quote(text, safe='')}"
    )
    req = Request(url, method="GET", headers={"Accept": "application/json", "User-Agent": "technocore-chat/1.2"})
    with urlopen(req, timeout=REQUEST_TIMEOUT) as response:
        raw = response.read().decode("utf-8")
    payload = json.loads(raw)
    if not isinstance(payload, dict):
        raise ValueError("Server returned an invalid send response.")
    return payload
...
class TechnocoreChat(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        yield Static("/help   /tutorial   /from   /seen   /full   /quit", id="hint")
        yield Footer()
    ...
    def load_initial_messages(self) -> None:
        try:
            payload = read_room(self.room, limit=READ_LIMIT)
    ...
    def handle_poll_error(self, exc: Exception) -> None:
        self.connected = False
        self.update_connection("● Reconnecting...")
    ...
    def process_messages(self, payload, initial=False) -> None:
        if not isinstance(payload, dict):
            self.write_message("Received an invalid server response.")
            return
        room_messages = payload.get("messages", [])
        if not isinstance(room_messages, list):
            self.write_message("Server returned invalid message data.")
            return
        normalized = []
        ...
        if initial:
            self.write_message("")
            self.write_message(f"Loaded {len(normalized)} public messages.")

    def format_message(self, timestamp: str, sender: str, text: str) -> str:
        display_sender = short_did(sender, self.seen_dids)
        if sender == self.did:
            display_sender = "You"
    ...
    def update_connection(self, text: str) -> None:
        if not self.running:
            return
        connection = self.query_one("#connection", Label)
    ...
    def send_message(self, text: str) -> None:
        thread = threading.Thread(target=self.send_worker, args=(text,), name="technocore-sender", daemon=True)
        thread.start()
