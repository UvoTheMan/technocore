...
class TechnocoreChat(App):
    ...
    def process_messages(self, payload, initial=False, force_latest=False) -> None:
        try:
        ...
        with self.queue_lock:
            blocked_seqs = self.displayed_seqs | self.queued_seqs
        for seq, message in normalized:
            if (not force_latest and seq <= current_last_seq) or seq in blocked_seqs:
                continue
        ...
        with self.queue_lock:
            for item in to_queue:
                seq = item[0]
                if seq in self.displayed_seqs or seq in self.queued_seqs:
                    continue
    ...
    def send_worker(self, text: str) -> None:
        try:
            ...
            if self.running:
                self.call_from_thread(self.handle_send_success, payload, text, room_at_send)
        except HTTPError as exc:
    ...
    def handle_send_success(self, payload, sent_text: str, room_at_send: str) -> None:
